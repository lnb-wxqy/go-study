...
type FaceObject struct {
	FaceID            string `json:"FaceID" validate:"required||string=48||regexp=21-22,02"`                      //人脸标识 正则匹配：^\d{20}02\d{26}$
	FaceAppearTime    string `json:"FaceAppearTime"`              //人脸出现时间
	FaceDisAppearTime string `json:"FaceDisAppearTime"`           //人脸消失时间
	Attitude          string `json:"Attitude,omitempty"`          //姿态分布
	Similaritydegree  int    `json:"Similaritydegree,omitempty"`  //相似度
	EyebrowStyle      string `json:"EyebrowStyle,omitempty"`      //眉形
	NoseStyle         string `json:"NoseStyle,omitempty"`         //鼻型
	MustacheStyle     string `json:"MustacheStyle,omitempty"`     //胡型
	LipStyle          string `json:"LipStyle,omitempty"`          //嘴唇
	WrinklePouch      string `json:"WrinklePouch,omitempty"`      //皱纹眼袋
	AcneStain         string `json:"AcneStain,omitempty"`         //痤疮色斑
	FreckleBirthmark  string `json:"FreckleBirthmark,omitempty"`  //黑痣标记
	ScarDimple        string `json:"ScarDimple,omitempty"`        //疤痕酒窝
	OtherFeature      string `json:"OtherFeature,omitempty"`      //其他特征
	PersonID          string `json:"PersonID,omitempty"`          //人员标识
	MotorVehicleID    string `json:"MotorVehicleID,omitempty"`    //机动车标识
	NonMotorVehicleID string `json:"NonMotorVehicleID,omitempty"` //非机动车标识
	ShotTime          string `json:"ShotTime,omitempty"`          //拍摄时间
	Emotion           int32  `json:"Emotion,omitempty"`           //人脸表情
	...
	InfoKind                          int     `json:"InfoKind,omitempty" validate:"in=1,0"`                          //信息分类 -备注：人工采集还是自动采集 R -
	...
	LeftTopX                          int32   `json:"LeftTopX,omitempty" validate:"required||integer=0,_"`                          //左上角X坐标
	LeftTopY                          int32   `json:"LeftTopY,omitempty"`                          //左上角Y坐标
	RightBtmX                         int32   `json:"RightBtmX,omitempty"`                         //右下角X坐标
	RightBtmY                         int32   `json:"RightBtmY,omitempty"`                         //右下角Y坐标
	...
	GenderCode                        string  `json:"GenderCode,omitempty"`                        //性别代码
	...
	IsDriver                          int32   `json:"IsDriver,omitempty"`                          //是否驾驶员
	IsForeigner                       int32   `json:"IsForeigner,omitempty"`                       //是否涉外人员
	...
	IsSuspectedTerrorist              int32   `json:"IsSuspectedTerrorist" validate:"required||in=0,1,2"`                        //是否涉恐人员
	SuspectedTerroristNumber          string  `json:"SuspectedTerroristNumber,omitempty"`          //涉恐人员编号
	IsCriminalInvolved                int32   `json:"IsCriminalInvolved" validate:"required||in=0,1,2"`                          //是否涉案人员
	...
	IsDetainees                       int32   `json:"IsDetainees" validate:"required||in=0,1,2"`                                 //是否在押人员
	...
	IsVictim                          int32   `json:"IsVictim" validate:"required||in=0,1,2"`                                    //是否被害人
	...
	IsSuspiciousFace                  int32   `json:"IsSuspiciousFace" validate:"required||in=0,1,2"`                            //是否可疑人
	...
	SubImageList *SubImageList `json:"SubImageList,omitempty"` //图像列表
	FeatureList  *FeatureList  `json:"FeatureList,omitempty"`  //特征值列表
	RelatedList  *RelatedList  `json:"RelatedList,omitempty"`  //关联关系实体
