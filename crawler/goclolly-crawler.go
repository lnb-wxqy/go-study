...
func main2() {
	...
	c1.OnHTML("ul[class= 'note-list']", func(e *colly.HTMLElement) {
		...
		e.ForEach("li", func(i int, element *colly.HTMLElement) {
			...
			err := c2.Request("GET", jhurl+href, nil, ctx, nil)
			if err != nil {
				fmt.Println("err: ", err)
			}
		})
	...
	// 设置登录cookie
	c1.SetCookies(jhurl, []*http.Cookie{
		&http.Cookie{
